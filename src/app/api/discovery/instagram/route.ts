...
export async function POST(request: NextRequest) {
  try {
    ...
    const { creators, postsMap } = await apifyService.discoverCreatorsByInstagramHashtag(hashtags, maxResults);
    ...
    return NextResponse.json({
      ...
      message: `Discovered and scored ${qualifiedCreators.length} creators.`,
    });

  } catch (error) {
    console.error('Apify Instagram discovery error:', error);
    
    return NextResponse.json({
      error: 'Instagram discovery failed via Apify',
